...
/**
 * Statement provider for service [pcs](https://docs.aws.amazon.com/service-authorization/latest/reference/list_pcs.html).
 *
...
export class Pcs extends PolicyStatement {
   ...
   * - .toCreateQueue()
   * - .toDeleteCluster()
   * - .toDeleteComputeNodeGroup()
   * - .toDeleteQueue()
   * - .toGetCluster()
   * - .toGetComputeNodeGroup()
   * - .toGetQueue()
   * - .toListComputeNodeGroups()
   * - .toListQueues()
   * - .toRegisterComputeNodeGroupInstance()
   * - .toTagResource()
   * - .toUntagResource()
   * - .toUpdateCluster()
