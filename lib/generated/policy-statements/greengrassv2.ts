...
/**
 * Statement provider for service [greengrass-v2](https://docs.aws.amazon.com/service-authorization/latest/reference/list_greengrassv2.html).
 *
...
export class GreengrassV2 extends PolicyStatement {
   ...
   * - .ifAwsResourceTag()
   */
  public onDeployment(groupId: string, deploymentId: string, partition2: string, region2: string, account2: string, deploymentId2: string, account?: string, region?: string, partition?: string) {
    return this.on(`arn:${ partition ?? this.defaultPartition }:greengrass:${ region ?? this.defaultRegion }:${ account ?? this.defaultAccount }:/greengrass/groups/${ groupId }/deployments/${ deploymentId }, arn:${ partition2 }:greengrass:${ region2 }:${ account2 }:deployments:${ deploymentId2 }`);
  }
